...
function shiftToLocation(loc) {
  ...
  if (typeof loc == "number") loc = map.locs[loc];
...
function clearTexts() {
  if (window.texts)
    for (var i = texts.length - 1; i >= 0; --i)
      if (texts[i]) removeChildSafe(texts[i], body);
  window.texts = [];
}
...
function spawnMap() {
  ...
  while (
    arrlen > current &&
    screenright >= (prething = arr[current]).xloc * unitsize
  ) {
    thing = prething.object;
    addThing(
      thing,
      prething.xloc * unitsize - gamescreen.left,
      prething.yloc * unitsize
    );
  ...
  while (
    arrlen > current &&
    screenrightpq >= (prething = arr[current]).xloc * unitsize
  ) {
    thing = prething.object;
    addThing(
      thing,
        ...
        if (me.top <= stopheight) {
          ...
          setTimeout(function () {
            ...
            setTimeout(function () {
              ...
              me.running = TimeHandler.addSpriteCycle(
                me,
                ["one", "two", "three", "two"],
                "running",
...
function pushPreScenery(name, xloc, yloc, repx, repy) {
  repx = round(repx || 1);
  repy = round(repy || 1);
  var prething = new PreThing(map.refx + xloc, map.refy - yloc, Sprite, name, [
    repx,
    repy,
  ]);
...
function makeCeiling(xloc, num) {
  num = num || 1;
  for (var i = 0; i < num; ++i) pushPreThing(Brick, xloc + i * 8, ceillev);
}
...
function pushPreWarpWorld(xloc, yloc, worlds, offset, block) {
  ...
  for (i = 0; i < len; ++i) {
    if (worlds[i] != -1) {
      warp.pipes.push(
        (pipe = pushPrePipe(startx, yloc, 24, true, worlds[i]).object)
      );
      warp.pirhanas.push(pipe.pirhana);
      if (worlds[i] instanceof Array)
        warp.texts.push(
          pushPreText(
            { innerText: worlds[i][0], style: { visibility: "hidden" } },
            startx + 4,
            38
          ).object
        );
...
function endCastleOutside(xloc, yloc, castlevel, wall, dist) {
  ...
  var detect = pushPreThing(FlagDetector, xloc + 7, yloc + 108).object,
    detect2 = pushPreThing(
      CastleDoorDetector,
      xloc + 60 + (castlev == 0) * 8,
      8
    ).object;
  detect.flag = pushPreThing(Flag, xloc + 0.5, yloc + 79.5).object;
...
// This new function handles the dialogue display when the player reaches the NPC.
// It creates a centered dialogue box on the screen.


// This is the final, consolidated function for all castle endings.
// It has been updated to use the new dialogue system.
function endCastleInsideFinal(xloc, characterName) {
  ...
  // The x and y properties have been removed as they are no longer needed.
  switch (characterName) {
    case "Peach":
      pushPreThing(Peach, xloc + 194, 12).object;
      stopper.dialogueData.push({ text: "THANK YOU " + window.player.title.toUpperCase() + "!" });
      stopper.dialogueData.push({ text: "YOUR QUEST IS OVER.<BR>WE PRESENT YOU A NEW QUEST." });
      stopper.dialogueData.push({ text: "PRESS BUTTON B<BR>TO SELECT A WORLD." });
      break;

    case "Luigi":
      pushPreThing(LuigiNPC, xloc + 194, 8).object;
      stopper.dialogueData.push({ text: "HEY, BRO!" });
      stopper.dialogueData.push({ text: "YOU HANDLED THAT PRETTY WELL!" });
      break;

    case "Goomba":
      npc = pushPreThing(Goomba, xloc + 194, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "..." });
      stopper.dialogueData.push({ text: "NO PRINCESS.<BR>ONLY GOOMBA." });
      break;

    case "Koopa":
      npc = pushPreThing(Koopa, xloc + 194, 12).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "WHOA, THANKS!" });
      stopper.dialogueData.push({ text: "I THOUGHT I WAS A GONER!<BR>...WAIT, YOU'RE NOT ONE OF US!" });
      break;

    case "HammerBro":
      npc = pushPreThing(HammerBro, xloc + 194, 12).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "HEY, WATCH THE HELMET!" });
      stopper.dialogueData.push({ text: "THE PRINCESS LEFT<BR>AGES AGO, PAL." });
      break;

    case "Bowser":
      npc = pushPreThing(Bowser, xloc + 190, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "FOOL!" });
      stopper.dialogueData.push({ text: "THAT WAS MY DECOY BROTHER!<BR>YOU'LL NEVER DEFEAT ME!" });
      break;

    case "Blooper":
      npc = pushPreThing(Blooper, xloc + 194, 12).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "*bloop bloop*" });
      stopper.dialogueData.push({ text: "(THE PRINCESS IS<BR>SOMEWHERE DRY!)" });
      break;

    case "CheepCheep":
      npc = pushPreThing(CheepCheep, xloc + 194, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "*glub glub*" });
      stopper.dialogueData.push({ text: "(WRONG CASTLE,<BR>BUB.)" });
      break;

    case "Lakitu":
      npc = pushPreThing(Lakitu, xloc + 194, 12).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "NICE AXE SKILLS!" });
      stopper.dialogueData.push({ text: "TOO BAD I ALREADY<BR>FLEW THE PRINCESS AWAY!" });
      break;

    case "Spiny":
      npc = pushPreThing(Spiny, xloc + 194, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "OW." });
      stopper.dialogueData.push({ text: "THE PRINCESS TOLD ME<BR>NOT TO TALK TO STRANGERS." });
      break;

    case "Beetle":
      npc = pushPreThing(Beetle, xloc + 194, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "*Stares blankly*" });
      stopper.dialogueData.push({ text: "SHE'S NOT HERE. SURPRISE." });
      break;

    case "Pirhana":
      npc = pushPreThing(Pirhana, xloc + 194, 12).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "*chomp chomp*" });
      stopper.dialogueData.push({ text: "I ATE THE PRINCESS.<BR>...JUST KIDDING." });
      break;

    case "Podoboo":
      npc = pushPreThing(Podoboo, xloc + 194, 8).object;
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "*fwoosh*" });
      stopper.dialogueData.push({ text: "THE LAVA IS WARMER<BR>IN THE NEXT CASTLE." });
      break;
    ...
    case "BulletBill":
      ...
      freezeCharacterAsNPC(npc);
      stopper.dialogueData.push({ text: "..." });
      stopper.dialogueData.push({ text: "SHE FLEW AWAY.<BR>I'M JEALOUS." });
      break;

    default: // This will be Toad
      pushPreThing(Toad, xloc + 194, 12).object;
      stopper.dialogueData.push({ text: "THANK YOU " + window.player.title.toUpperCase() + "!" });
      stopper.dialogueData.push({ text: "BUT OUR PRINCESS IS IN<BR>ANOTHER CASTLE!" });
      break;
  }
}

function pushPreSectionPass(xloc, yloc, width, height, secnum) {
  var passer = pushPreThing(
      Collider,
      xloc,
      yloc,
      [width, height],
...
function activateSection(parent, status) {
  ...
  for (var i = colliders.length - 1; i >= 0; --i) killNormal(colliders[i]);
...
function pushPreFuncCollider(position, func) {
  ...
  if (position instanceof Array) {
    ...
    return pushPreThing(FuncCollider, position[0], position[1], func, [
      position[2],
      position[3],
    ]);
  }
...
function World11(map) {
  ...
  map.areas = [
    new Area("Overworld", function () {
      ...
      var greeter = "";
      greeter +=
        "<div style='width:350px;max-height:189px;background-color:#d64d00;border-radius:7px;box-shadow:3px 3px #efb28b inset, -3px -3px black inset;";
      greeter +=
      ...
      greeter +=
        "background-position: 7px 7px, 336px 7px, 7px 168px, 336px 168px";
      greeter += "'>";
      greeter +=
      ...
      greeter +=
        "    <span style='font-size:81px;line-height:96px'>MARIO BROS.</span>";
      greeter += "  </p>";
      greeter += "</div>";
      greeter +=
...
function randMapType(map) {
  map.locs = [new Location(0, entryRandom)];
  map.areas = [
...
console.log(
  "This is an offline copy of Full Screen player, intended for private testing.",
...
function World42(map) {
  ...
  map.areas = [
    ...
    new Area("Underworld", function () {
      // main underworld - locs 1 (entryNormal), 2 (exitPipeVert)
      setLocationGeneration(1);
    ...
    new Area("Castle", function () {
      ...
      this.sections = {
        ...
        0: function (xloc) {
          ...
          pushPreThing(Platform, xloc + bstretch * 8 + 152, 0, 4, [
            moveSliding,
            xloc + bstretch * 8 + 140,
            xloc + bstretch * 8 + 232,
            2,
